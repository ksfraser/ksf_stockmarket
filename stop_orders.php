    ...
    <table>
        <thead>
            <tr>
                ...
                <th class="r" title="Suggested time-based stop expiry tied to ATR horizon">Time Stop</th>
                <th class="r" title="Risk-based suggested position size (1% portfolio risk)">Suggested Size</th>
                <th class="r" title="Catastrophic all-out stop (5× ATR below current)">All-Out Stop</th>
                <th title="Trend exhaustion: price dropped >3× ATR from 60d high or ATR stop distance >4×">Trend Exhaustion</th>
                <th class="r" title="Composite exit urgency 0-100 from oscillators + ATR multiple">Urgency</th>
        ...
        ?>
            <tr>
                ...
                <td class="r"><?= $o['suggested_time_stop_days'] ?? '—' ?>d</td>
                <td class="r"><?= ($o['suggested_shares'] ?? 0) > 0 ? number_format($o['suggested_shares'], 0) : '—' ?></td>
                <td class="r"><?= $o['all_out_stop_price'] !== null ? '$' . number_format($o['all_out_stop_price'], 2) : '—' ?></td>
                <td style="color:<?= ($o['trend_exhaustion'] ?? false) ? 'var(--red)' : 'var(--text3)' ?>; font-weight:700;"><?= ($o['trend_exhaustion'] ?? false) ? '⚠ EXHAUSTION' : '—' ?></td>
                <td class="r" style="color:var(--<?= ($o['exit_urgency_score'] ?? 0) >= 70 ? 'red' : (($o['exit_urgency_score'] ?? 0) >= 40 ? 'yellow' : 'green') ?>)"><?= $o['exit_urgency_score'] ?? '—' ?></td>
