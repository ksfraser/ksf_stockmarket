...
function testcandlesticksimplehalf( $symbol, $startingcash, $tradedatearray, $actionarray )
{
	...
	if( require_once( 'adduser.php' ))
	{
		$ret = adduser( $flog,
				...
				$firstname = "Buy Sell all stock or cash",
	...
	foreach( $ta->resultarray as $row )
	{
		...
		$transactiondate = $tradedatearray[$row['date']]; 
		...
		else if( $action == "buystock" )
		{
			...
			fwrite( $flog, "Have $" . $dollarsavailable . " with which to buy shares.  Using half\n" );
			$dollarsavailable /= 2;
