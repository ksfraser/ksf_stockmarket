...
<!-- Portfolio Performance (TWR/IRR/Drawdown/Volatility) -->
<?php
$perf = $data['portfolio_performance'] ?? [];
if (!empty($perf) && !isset($perf['error'])):
    $fmt = static fn (?float $v, int $dec=2) => ($v === null ? '—' : (($v >= 0 ? '+' : '') . number_format($v * 100, $dec) . '%'));
?>
<div class="card" style="border-color:var(--accent);">
    <div class="card-header">&#x1F4C8; Portfolio Performance (<?php echo date('Y-m-d', strtotime('-1 year')); ?> to <?php echo date('Y-m-d'); ?>)</div>
    <div class="stats-grid">
        <?php foreach ($perf as $acct => $m): ?>
        <div class="stat-card">
            <div class="stat-label" style="margin-bottom:8px;"><strong><?php echo htmlspecialchars($acct); ?></strong></div>
            <?php if (!empty($m['twr']) || !empty($m['irr'])): ?>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 12px;font-size:0.9em;">
                    <span>TWR:</span>
                    <span class="<?php echo (($m['twr']['twr'] ?? 0) >= 0) ? 'pnl-positive' : 'pnl-negative'; ?>">
                        <?php echo $fmt($m['twr']['twr'] ?? null, 1); ?>
                        <?php if (!empty($m['twr']['annualized_twr'])): ?>
                            (ann. <?php echo $fmt($m['twr']['annualized_twr'], 1); ?>)
                        <?php endif; ?>
                    </span>
                    <span>IRR:</span>
                    <span class="<?php echo (($m['irr']['irr'] ?? 0) >= 0) ? 'pnl-positive' : 'pnl-negative'; ?>">
                        <?php echo $fmt($m['irr']['irr'] ?? null, 1); ?>
                    </span>
                    <span>Max DD:</span>
                    <span class="pnl-negative">
                        <?php echo $fmt(-($m['drawdown']['max_drawdown'] ?? 0), 1); ?>
                    </span>
                    <span>Vol (ann):</span>
                    <span>
                        <?php echo $fmt($m['volatility']['annualized_std_dev'] ?? null, 1); ?>
                        <?php if (!empty($m['volatility']['semi_deviation'])): ?>
                            / <?php echo $fmt($m['volatility']['semi_deviation'] ?? null, 1); ?> (down)
                        <?php endif; ?>
                    </span>
                </div>
            <?php else: ?>
                <p class="text-muted" style="font-size:0.85em;">Insufficient data for 1-year window</p>
            <?php endif; ?>
        </div>
        <?php endforeach; ?>
    </div>
