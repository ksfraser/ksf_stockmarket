...
class UserController {
    ...
    public function myDashboard(): array {
        ...
        // Portfolio performance (TWR/IRR/drawdown/volatility via ksfraser/portfolio-math)
    ...
    /** Performance metrics per account via ksfraser/portfolio-math. */
    private function getPortfolioPerformance(int $userId): array
    {
        ...
        try {
            $pdo = Database::get();
            $stmt = $pdo->prepare("SELECT DISTINCT account_type FROM portfolio WHERE user_id = :uid AND account_type IS NOT NULL AND account_type != '' AND shares > 0");
            $stmt->execute([':uid' => $userId]);
            $accounts = array_column($stmt->fetchAll(), 'account_type');
            ...
            error_log('UserController::getPortfolioPerformance failed: ' . $e->getMessage());
