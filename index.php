...
case 'strategy_timing':
    ...
    case 'seg_fund_lira':
        ...
        $pageTitle = 'LIRA/LRSP Seg-Fund Screener — 10-Year Horizon';
    ...
    case 'seg_fund_lira_5y':
        ...
        $pageTitle = 'LIRA/LRSP Seg-Fund Screener — 5-Year Horizon';
    ...
    case 'etf_performance':
        ...
        $perfColumns = [
            ...
            '1Y' => 'p_1Y', '2Y' => 'p_2Y', '3Y' => 'p_3Y', '5Y' => 'p_5Y', '10Y' => 'p_10Y',
        ];
        $dirs = ['asc' => 'ASC', 'desc' => 'DESC'];

        if (isset($perfColumns[$sortField])) {
            $orderBy = $perfColumns[$sortField] . ' ' . ($dirs[$sortDir] ?? 'ASC');
        } elseif ($sortField === 'name') {
            $orderBy = 'eb.name ' . ($dirs[$sortDir] ?? 'ASC');
        } elseif ($sortField === 'category') {
            $orderBy = 'eb.category ' . ($dirs[$sortDir] ?? 'ASC');
        } elseif ($sortField === 'yield_12m') {
            $orderBy = 'eb.yield_12m ' . ($dirs[$sortDir] ?? 'ASC');
        } elseif ($sortField === 'expense_ratio') {
            $orderBy = 'eb.expense_ratio ' . ($dirs[$sortDir] ?? 'ASC');
        } else {
            $sortField = 'symbol';
        ...
        try {
            ...
            $sql = "
                WITH latest AS (
                    ...
                    FROM symbol_performance GROUP BY symbol
                ), pivot AS (
                    SELECT
                        sp.symbol,
                        MAX(CASE WHEN sp.timeframe = '1M'  THEN sp.net_return END) as p_1M,
                        MAX(CASE WHEN sp.timeframe = '3M'  THEN sp.net_return END) as p_3M,
                        MAX(CASE WHEN sp.timeframe = '6M'  THEN sp.net_return END) as p_6M,
                        MAX(CASE WHEN sp.timeframe = '9M'  THEN sp.net_return END) as p_9M,
                        MAX(CASE WHEN sp.timeframe = '12M' THEN sp.net_return END) as p_12M,
                        MAX(CASE WHEN sp.timeframe = '1Y'  THEN sp.net_return END) as p_1Y,
                        MAX(CASE WHEN sp.timeframe = '2Y'  THEN sp.net_return END) as p_2Y,
                        MAX(CASE WHEN sp.timeframe = '3Y'  THEN sp.net_return END) as p_3Y,
                        MAX(CASE WHEN sp.timeframe = '5Y'  THEN sp.net_return END) as p_5Y,
                        MAX(CASE WHEN sp.timeframe = '10Y' THEN sp.net_return END) as p_10Y,
                        MAX(sp.as_of_date) as as_of_date
                    FROM symbol_performance sp
                    JOIN latest l ON sp.symbol = l.symbol AND sp.as_of_date = l.max_date
                    GROUP BY sp.symbol
                )
                SELECT
                    p.symbol, p.as_of_date, eb.name, eb.category, eb.region,
                    eb.yield_12m, eb.expense_ratio,
                    p.p_1M, p.p_3M, p.p_6M, p.p_9M, p.p_12M,
                    p.p_1Y, p.p_2Y, p.p_3Y, p.p_5Y, p.p_10Y
                FROM pivot p
                LEFT JOIN etf_benchmarks eb ON eb.symbol = p.symbol
                ORDER BY {$orderBy}
            ";

            $stmt = $pdo->query($sql);
            $rows = $stmt->fetchAll();
            $asOf = $rows ? ($rows[0]['as_of_date'] ?? '-') : '-';
            $error = null;
        } catch (Exception $e) {
            $rows = [];
            $asOf = '-';
