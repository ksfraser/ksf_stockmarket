...
function testcandlesticksimple( $symbol, $startingcash, $tradedatearray, $actionarray )
{
	...
	foreach( $ta->resultarray as $row )
	{
		...
		if( $action == "buystock" or $action == "sellstock" )
		{
			...
			else if( $action == "sellstock" )
			{
				...
				$numbershares = CalculateSharesAvailable( $user, $symbol );
