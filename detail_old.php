...
<!-- Price Chart -->
<div class="card">
    <div class="card-header" title="Closing price over 250 trading days.">Price History (250 days)</div>
...
<!-- Fundamental Data -->
<?php if (!empty($data['fundamentals'])): ?>
<?php $f = $data['fundamentals']; ?>
<div class="card">
    <div class="card-header">Fundamental Data</div>
    <div class="grid-3">
        <div>
            <div class="ind-item" style="margin-bottom:8px">
                <div class="ind-name">Market Cap</div>
                <div class="ind-value"><?= $f['market_cap'] ? '$' . fmt_num($f['market_cap']) : 'N/A' ?></div>
            </div>
            <div class="ind-item" style="margin-bottom:8px">
                <div class="ind-name">Revenue (TTM)</div>
                <div class="ind-value"><?= $f['total_revenue'] ? '$' . fmt_num($f['total_revenue']) : 'N/A' ?></div>
            </div>
            <div class="ind-item" style="margin-bottom:8px">
                <div class="ind-name">Free Cash Flow</div>
                <div class="ind-value"><?= $f['free_cash_flow'] ? '$' . fmt_num($f['free_cash_flow']) : 'N/A' ?></div>
            </div>
            <div class="ind-item" style="margin-bottom:8px">
        ...
        <div>
            <div class="ind-item" style="margin-bottom:8px">
                <div class="ind-name">P/E Ratio (TTM)</div>
                <div class="ind-value"><?= $f['trailing_pe'] ? number_format($f['trailing_pe'], 2) : 'N/A' ?></div>
            </div>
            <div class="ind-item" style="margin-bottom:8px">
                <div class="ind-name">Forward P/E</div>
                <div class="ind-value"><?= $f['forward_pe'] ? number_format($f['forward_pe'], 2) : 'N/A' ?></div>
            </div>
            <div class="ind-item" style="margin-bottom:8px">
                <div class="ind-name">PEG Ratio</div>
                <div class="ind-value"><?= $f['peg_ratio'] ? number_format($f['peg_ratio'], 2) : 'N/A' ?></div>
            </div>
            <div class="ind-item" style="margin-bottom:8px">
                <div class="ind-name">Price/Book</div>
                <div class="ind-value"><?= $f['price_to_book'] ? number_format($f['price_to_book'], 2) : 'N/A' ?></div>
            </div>
        </div>
        <div>
            <div class="ind-item" style="margin-bottom:8px">
            ...
            <div class="ind-item" style="margin-bottom:8px">
                <div class="ind-name">Debt/Equity</div>
                <div class="ind-value" style="color:<?= ($f['debt_to_equity'] ?? 0) <= 1.5 ? 'var(--green)' : (($f['debt_to_equity'] ?? 0) <= 2 ? 'var(--yellow)' : 'var(--red)') ?>">
                    <?= $f['debt_to_equity'] !== null ? number_format($f['debt_to_equity'], 2) . '×' : 'N/A' ?>
                </div>
            </div>
            <div class="ind-item" style="margin-bottom:8px">
                <div class="ind-name">Current Ratio</div>
                <div class="ind-value"><?= $f['current_ratio'] ? number_format($f['current_ratio'], 2) : 'N/A' ?></div>
            </div>
        </div>
    </div>
...
<!-- Dividend Safety -->
<?php if (!empty($data['dividend_safety']) && $data['dividend_safety']['score'] !== null): ?>
<?php $ds = $data['dividend_safety']; ?>
<div class="card">
    <div class="card-header">
        Dividend Safety Score
        <span style="float:right; font-size:1.5em; color:<?= $ds['rating_color'] ?>"><?= $ds['score'] ?>/100 (<?= $ds['rating'] ?>)</span>
    </div>
    <?php if ($ds['score'] !== null): ?>
    <div class="bar" style="height:12px; margin-bottom:16px">
        <div class="bar-fill" style="width:<?= $ds['score'] ?>%; background:<?= $ds['rating_color'] ?>"></div>
    </div>
    <?php endif; ?>
    <?php foreach ($ds['components'] as $comp): ?>
        <div style="display:flex; justify-content:space-between; padding:4px 0; border-bottom:1px solid var(--border); font-size:0.9em">
            <span><?= htmlspecialchars($comp[0]) ?></span>
            <span style="color:<?= $comp[1] === 'OK' || $comp[1] === 'STRONG' ? 'var(--green)' : ($comp[1] === 'CRITICAL' ? 'var(--red)' : ($comp[1] === 'WARNING' ? 'var(--yellow)' : 'var(--text)')) ?>">
                <?= htmlspecialchars($comp[2]) ?>
            </span>
        </div>
    <?php endforeach; ?>
</div>
...
<!-- Dividend History -->
<?php if (!empty($data['dividends'])): ?>
<div class="card">
    <div class="card-header">Dividend History (Last <?= count($data['dividends']) ?> payments)</div>
    <table>
        <thead><tr><th>Ex-Date</th><th class="r">Amount</th></tr></thead>
        <tbody>
        <?php foreach (array_slice($data['dividends'], 0, 20) as $d): ?>
            <tr>
                <td><?= htmlspecialchars($d['ex_date']) ?></td>
                <td class="r">$<?= number_format($d['amount'], 4) ?></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
...
<?php endif; // end fundamentals check ?>
